...
class Modal extends Component<ModalProps, ModalState> {
  constructor(props: ModalProps) {
    super(props);
    this.state = {
      newTags: this.props.modals.tag_names
        ? this.props.modals.tag_names.join(" ")
        : "",
      oldTags: this.props.modals.tag_names
        ? this.props.modals.tag_names.join(" ")
        : "",
    };
  }
  ...
  handleNewTags = (e: React.MouseEvent<HTMLButtonElement>): void => {
    const { add_tags, delete_tags } = this.compareTagArrays(
      this.state.oldTags.split(" "),
      this.state.newTags.split(" ")
    );
    const tag_names = this.props.modals.tag_names;

    if (tag_names.length === 0)
      this.props.postNewTags(this.props.modals.reading_id, add_tags);
    else
      this.props.updateTags(
        this.props.modals.reading_id,
        add_tags,
        delete_tags
      );
    ...
    return (
      <>
        ...
        <small className="form-text text-muted">
          Add, update, and delete tags for this reading above. Separate tags
          with '#'. (e.g. #fun #learning)
        </small>
        {oldTags === newTags ? (
          <button
            onClick={this.handleNewTags}
            type="button"
