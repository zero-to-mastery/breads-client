...
const getTagReadings = (readings: any[], tag_id: number): any[] => {
  return readings.filter((reading) => {
    ...
    const containsTagId =
      reading.tags === tag_id || (notNull && reading.tags.includes(tag_id));
...
export const getReadings = (
  ...
  if (
    state.readingsByList[`${list}`] &&
    state.readingsByList[`${list}`].items
  ) {
    let readings = state.readingsByList[`${list}`].items
      .map((id: number) => state[NAME][id])
      .reverse();
...
export const getWebsites = (
  ...
  if (
    ...
    let readings = state.readingsByList[`${list}`].items.map(
 ...
 * @todo update prop types once readingsByList state is type checked
 */
export const getUserReadingsInNeedOfUpdate = (
  state: any,
  list: string
): any[] | void => {
  // get readings by list (wait for list to load)
  if (
    ...
    const readings = state.readingsByList[`${list}`].items
      .map((id: number) => state["readings"][id])
      .reverse(); // NAME from readings constants
    const updateConditions = [
      "403 ",
      "Are you a robot",
      "Bloomberg",
      "Unable to get title of article",
      "Error 404",
    ];

    // if title in conditions list, return reading
    const titleCheck = (reading: any) =>
      updateConditions.some((r) => reading.title.includes(r));

    // if description and image are blank, return reading
    const descriptionAndImageCheck = (reading: any) =>
      reading.description === null && reading.reading_image === null;

    const outdatedReadingsFromTitle = readings.filter((reading: any) =>
      titleCheck(reading)
    );
    const outdatedReadingsFromOther = readings.filter((reading: any) =>
      descriptionAndImageCheck(reading)
    );
