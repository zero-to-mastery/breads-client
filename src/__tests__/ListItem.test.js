...
let [viewSummary, removeSummary, newSubscription, removeReading] = new Array(
  4
).fill(jest.fn());
...
describe("Shallow rendered ListItem", () => {
  it("renders self", () => {
    ...
    expect(
      enzymeWrapper.find("div.reading-area").find("p.text-muted").text()
    ).toBe(`~${Number(props.word_count).toLocaleString()} words`);
    expect(enzymeWrapper.find("img").hasClass("timeline-image")).toBe(true);
    expect(enzymeWrapper.find("img").html()).toContain(props.image);
    expect(enzymeWrapper.find("p.btn.text-primary.m-2").text()).toBe(
      props.username
    );
  });
});
